\chapter{Condiciones Fritz John y Karush-Kuhn-Tucker}
		\newcommand{\barx}{\bar{x} }
		\newcommand{\dd}{\textbf{\emph{d}}}
		
	En primer lugar, empezamos recordando la definición de derivada direccional. 
	\begin{definicion}
			Sea $ D \subset \vecSpace $ subconjunto de un espacio vectorial y sea la función $ g: D \longrightarrow  \RR$ definimos la derivada direccional de g en la dirección del vector $ d\in \vecSpace $ como
			\[
			g'(x;d) = \lim_{t\rightarrow0}\frac{g(x+td) - g(x)}{t}
			\]
			siempre y cuando el límite exista. Diremos que g es diferenciable en el sentido de Gâteaux en x si $ g'(x;d) $ es lineal en d, es decir, $ g'(x;d) = \langle \nabla g(x), d\rangle $.
	\end{definicion}

	Dentro de este contexto, cuando decimos que una función es diferenciable nos referimos a que es diferenciable en el sentido de Gâteaux. Antes de continuar demostremos cómo se calcula la derivada de la función máximo, lo que nos será útil en posteriores resultados.
	
	\begin{proposicionBox}\label{dirDeriv}
		Sea $ D \subset \vecSpace$ un subconjunto de un espacio vectorial, $ \barx $ un punto del interior de $ D $ y sean $ g_1, ..., g_N : D \longrightarrow \RR $  funciones continuas y diferenciables en $ \barx $ donde $ N \in \NN $. Definimos $ g:D \longrightarrow \RR $ como $ g(x):=\max_{i=1,...,N}\{g_i(x)\} $ y el conjunto de índices $ K = \lbrace  i : g_i(\barx) =  g(\barx) \rbrace $. Entonces, para toda $ d \in \vecSpace $ dirección del espacio vectorial, la derivada direccional de $ g $ viene dada por la siguiente expresión:
		\begin{equation}
	...
	Nuestro objetivo ahora es encontrar soluciones a problemas del siguiente tipo:
		...
		\begin{proof}
			...
			\]
			Como $ \barx $ es un mínimo local del problema \ref{probMin} también lo es de $ g $. Esto se debe a que como $ f(x) \geq f(\barx) $ entonces $ f(x) - f(\barx) \geq 0$ para todo $ x $ en un entorno de $ \barx $. Por otro lado, $ g_i(x) \leq 0 $ si $ x \in C $ y si $ i \in I(\barx) $ entonces $ g_i(\barx)=0 $. De este modo $ g(x) \geq 0 \text{ } $ para todo $ x $ en un entorno de $ \barx $ y $ g(\barx) = 0 $ por lo que efectivamente alcanza un mínimo local en $ \barx $. Por la proposición \ref{dirDeriv} tenemos que para toda dirección d del espacio vectorial se cumple:
			\[
