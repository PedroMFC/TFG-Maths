...
\begin{figure}[h!]\label{graphPut}
	...
	\begin{minipage}{0.5\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{Buyer_put} 
		%\caption{Tras prealineado.}
		%\label{fig:subim1}
	\end{minipage}
	\begin{minipage}{0.5\textwidth}
		\includegraphics[width=1\linewidth]{Writer_put}
		%\caption{Tras cuatro iteraciones.}
...
La cartera de inversión o portafolios para el instante $ t \in \mathbb{T} $ viene dada por la variable aleatoria $ d+1 $ dimensional $ \theta_t = (\theta_t^0,\dots,\theta_t^d) $. Su valor viene determinado por $ V_t(\theta) $ donde
\[
...
Exponemos ahora unos resultados que nos servirán posteriormente para probar el teorema principal del trabajo.
